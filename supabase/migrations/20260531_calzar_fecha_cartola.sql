-- ============================================================
-- La fecha de la cartola bancaria manda al calzar
-- ============================================================
-- Principio rector: la fecha del movimiento bancario es la única
-- fuente de verdad de cuándo se hizo efectivo un ingreso. Las fechas
-- manuales son estimaciones previas a la conciliación; una vez que
-- existe el movimiento bancario, la del banco manda (así los socios
-- pueden cuadrar sus pagos contra su propia cartola).
--
-- Esta migración RE-CREA calzar_movimiento_con_ingreso (no edita la
-- migración 20260531_conciliacion_calce.sql, ya aplicada). Además de
-- todo lo que ya hacía, sobrescribe la fecha del ingreso manual con la
-- fecha del movimiento bancario:
--   - pagos_cuota:    fecha_pago     = v_mov.fecha
--   - cheques:        fecha_deposito = v_mov.fecha
--   - otros_ingresos: fecha          = v_mov.fecha
-- El insert de otros_ingresos (caso "cheque concepto otro") ya usaba
-- v_mov.fecha, así que ese caso queda igual.
--
